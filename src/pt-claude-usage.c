 ...
 * tried. Both shapes have used more than one spelling across versions. */
static const char *const percent_keys[] = {
  "utilization", "used_percent", "percent_used", "usage_percent", "percentage",
};
...
/* The window's name as the user knows it. `key` is the member name in the old
 * shape or the entry's type in the new one; `model` is the new shape's way of
 * saying which model a weekly cap applies to. */
...
static gboolean parse_limits_array(JsonArray *limits, PtUsage *u) {
  ...
  for (guint i = 0; i < n && u->n_windows < PT_USAGE_MAX_WINDOWS; i++) {
    ...
    const char *key = pt_json_string(w, "type");
    ...
    char label[64];
    limit_label(key, pt_json_string(w, "model"), label, sizeof(label));
